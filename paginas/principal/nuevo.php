    ...
    <div class="page-content">
		...
		<form class="row g-3" method="POST" action="guarda.php" enctype="multipart/form-data" autocomplete="off">
			...
			<div class="col-md-6">
				...
				<select name="tipo_instalacion" id="tipo_instalacion" class="form-select" required>
                    ...
                    if(file_exists($jsonFileTypes)) {
                        ...
                        if(is_array($typesData)) {
                            foreach($typesData as $type) {
                                // Se asume monto 0 ya que ahora es editable dinámicamente
                                echo '<option value="' . $type . '" data-monto="0">' . $type . '</option>';
                            }
                        }
                    } else {
                         // Fallback básico
			...
			<div class="col-md-6">
				...
				<input type="number" step="0.01" class="form-control" id="monto_instalacion" name="monto_instalacion">
			</div>

			<div class="col-md-6">
				<label for="gastos_adicionales" class="form-label">Gastos Adicionales ($)</label>
				<input type="number" step="0.01" class="form-control" id="gastos_adicionales" name="gastos_adicionales" value="0">
			</div>
			...
			<div class="col-md-6">
				...
				<input type="number" class="form-control" id="dias_prorrateo" name="dias_prorrateo" value="0" placeholder="0">
			</div>

			<div class="col-md-6">
				<label for="monto_prorrateo_usd" class="form-label">Monto Prorrateo ($)</label>
				<input type="number" step="0.01" class="form-control" id="monto_prorrateo_usd" name="monto_prorrateo_usd" readonly>
			</div>

			<div class="col-md-6">
				<label for="monto_pagar" class="form-label">Monto Total a Pagar ($)</label>
				<input type="number" step="0.01" class="form-control" id="monto_pagar" name="monto_pagar" readonly>
			</div>
			...
			<div class="col-md-12">
				...
				<input type="file" class="form-control" id="evidencia_foto" name="evidencia_foto" accept="image/*">
			</div>

			<div class="col-md-6">
				<?php
				$sql_instaladores = "SELECT * FROM instaladores WHERE activo = 1 ORDER BY nombre_instalador ASC";
				$resultado_instaladores = $conn->query($sql_instaladores);
				?>
				<label for="instaladores" class="form-label font-weight-bold">Instaladores</label>
				<select name="instaladores[]" id="instaladores" class="form-select" multiple size="4">
					<?php
					if ($resultado_instaladores->num_rows > 0) {
						while($inst = $resultado_instaladores->fetch_assoc()) {
							echo '<option value="' . $inst['id_instalador'] . '">' . 
								 htmlspecialchars($inst['nombre_instalador']) . '</option>';
						}
					} else {
